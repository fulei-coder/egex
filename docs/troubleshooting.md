...
### U-arm 话题无数据（`/servo_angles`）

**现象**：采集脚本可以启动，但按 `c` 校准失败，提示 leader 无数据。
...
- 无输出：先检查 `servo_reader.py` 的串口和波特率参数。
- 输出维度 < 7：检查上游节点是否被改动。
...
**现象**：HDF5 中 `qpos/action` 是 `(N,7)`，但第7维几乎恒定。

**结论**：这是设计行为，不是故障。当前阶段只执行6关节，第7维是夹爪占位接口。

**何时会变化**：启用 `gripper.enabled=true` 且控制链路接入真实二指夹爪后。

---

### 误触发夹爪通信导致频率下降

**现象**：按计划应为 6DoF 调试，但循环频率异常下降。

**原因**：配置中误开夹爪执行，触发 Modbus 通信。

**解决**：

- 确认配置 `gripper.enabled=false`
- 推理脚本不传 `--execute-gripper`
